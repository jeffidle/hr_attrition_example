...
# Model accuracies
model_et_accuracy_nb <- mean(predictions_nb$class == test_data_df$Attrition)
model_et_accuracy_lr <- mean(predictions_lr == test_data_df$Attrition)
model_et_accuracy_rf <- mean(predictions_rf == test_data_df$Attrition)
model_et_accuracy_rt <- mean(predictions_rt == test_data_df$Attrition)
...
We will performance-tune the logistic regression model because it:  
  ...
  3. has the highest balanced accuracy;
  4. has one of the lowest accuracy p-values; and
  5. gives us more information about the levels within our factor variables than the other models.
...
likelihood_print_df <- likelihood_df %>%
...
ggplot(feature_select_df, aes(x = attribute_count, y = mcfadden_r, color = significant)) +
        geom_line(size = 4) +
        labs(title = "McFadden R increase with addition of attribute to model", subtitle = "(significant and not-significant attributes)", x = "attribute count", y = "") +
        scale_color_manual(values = custom_palette[c(4, 1)]) +
        theme_minimal()

```

Given the preceding analysis, we will use the 20 significant variables in our model and drop the remaining 7 non-significant variables. The last step before we build our final model will be to assign cost consequences. For our predictor model there are two types of errors:  1) an active employee is incorrectly predicted to be a retention risk; and 2) a retention risk employee is predicted to be a non-retention risk. We want to minimize the errors in which true retention-risk employees are classified as non-retention risk employees because we miss the opportunity to intervene before the employee leaves. If we mis-classify an active employee as a retention risk, then our loss is only the time and effort we spent trying to retain someone who did not intend to leave. 

We will assign greater costs to errors that mis-classify true retention risks. We will make an assumption that incorrect classifications of true retention risks are 8 times more costly than incorrect classifications of true non-retention risks.
...
# Build the models >> note that we use the full dataset (not the 80/20 split) because we are
# using the cross-validation method
model_cost_cv_lr_df <- train(Attrition ~ ., data = final_model_data_df[, -1], method = "glm", family = "binomial",
                          trControl = trainControl("cv", number = 10))

# Make predictions
predicted_classes_cost_cv_lr <- model_cost_cv_lr_df %>% predict(final_model_data_df)

# Calculate the confusion matrices
model_cost_cv_cm_lr <- confusionMatrix(predicted_classes_cost_cv_lr, final_model_data_df$Attrition)

cm_cv_cost_lr <- data.frame(as.matrix(model_cost_cv_cm_lr$table)) %>%
        mutate(algorithm = "logistic_regression")

names(cm_cv_cost_lr) <- c("predicted", "actual", "n_cost", "algorithm")

cm_cv_cost_lr <- cm_cv_cost_lr %>%
        mutate(pct_cost = n_cost / sum(n_cost, na.rm = TRUE)) %>%
        mutate(outcome = ifelse(actual == predicted, "correct", ifelse(actual == "active" & predicted == "termed", "false negative", ifelse(actual == "termed" & predicted == "active", "false positive", "unknown")))) %>%
        dplyr::select(actual, predicted, outcome, n_cost, pct_cost, algorithm) %>%
        arrange(algorithm, outcome, actual, predicted)

cm_cv_cost_lr$phase <- "cost_controlled_cross_validation"

cm_cv_cost_print_lr <- cm_cv_cost_lr %>%
        dplyr::select(-algorithm, -phase)

cm_cv_cost_print_lr %>%
        gt() %>%
        tab_spanner(label = md("*Cost-controlled cross-validation*"), columns = vars(n_cost, pct_cost)) %>%
        cols_label(n_cost = "n", pct_cost = "%") %>%
        fmt_percent(c(5), decimals = 1) %>%
        tab_options(table.font.size = 12,
                    row.padding = px(1)
                    )
...
metric_cost_labels <- row.names(as.data.frame(c(model_cost_cv_cm_lr$overall, model_cost_cv_cm_lr$byClass)))

perf_metrics_cost_cv_lr <- data.frame(metric = metric_cost_labels,
                                        metric_result = c(model_cost_cv_cm_lr$overall, model_cost_cv_cm_lr$byClass)) %>%
        mutate(algorithm = "logistic_regression",
               phase = "cost_controlled_cross_validation")

names(perf_metrics_cost_cv_lr) <- c("metric", "metric_result_lr_cost", "algorithm", "phase")

perf_metrics_cost_cv_lr %>%
        gt() %>%
        cols_label(metric_result_lr_cost = "cost-controlled cross-validation") %>%
        fmt_percent(c(2), decimals = 1) %>%
        tab_options(table.font.size = 12,
                    row.padding = px(1)
                    )
