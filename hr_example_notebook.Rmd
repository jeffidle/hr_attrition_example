...
<br>
<br>
Let's toss out the factors that are not significant (p-value > 0.05). That leaves us with 20 factors. That's still a lot of variables to include in our model, but they could all add value. Let's take a look at the goodness-of-fit measures for all importance threshold cutoff points. The goodness-of-fit measures we will analyze are:  1) likelihood ratio test; 2) pseudo R-squared; and 3) the Hosmer-Lemeshow Test.

Likelihood ratio discussion

Pseudo R squared discussion

Hosmer-Lemeshow Test discussion
...
feature_list <- unique(features_selected1_df$factor)
...
ggplot(pseudo_r_df, aes(x = attribute_count, y = mcfadden_r)) +
        geom_line() +
        theme_minimal()
