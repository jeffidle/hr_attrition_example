...
```{r likelihood_ratio_calc, echo=FALSE, message=FALSE, warning=FALSE, include=TRUE}
...
```{r pseudo_r_count, include=TRUE, echo=FALSE, message=FALSE, warning=FALSE}
...
Compare all model confusion matrices here:
```{r echo=FALSE, message=FALSE, warning=FALSE}

```

Compare all model performance metrics here:
```{r echo=FALSE, message=FALSE, warning=FALSE}

```

All user to select two models and compare ROC curves here:
...
predicted_probs_rf <- model_rf %>% predict(test_data_df, type = "response")
...
pred_lr_80 <- prediction(predictions =  predicted_probs_lr$termed, labels = test_data_df$Attrition)
pred_rf_80 <- prediction(predictions =  predicted_probs_rf$termed, labels = test_data_df$Attrition)
pred_rt_80 <- prediction(predictions =  predicted_probs_rt$termed, labels = test_data_df$Attrition)
pred_nb_cv <- prediction(predictions = predicted_probs_nb_cv$posterior.termed, labels = modeling_data_df$Attrition)
...
roc_viz <- ggplot(roc_df, aes(x = false_positive_rate, y = true_positive_rate, color = model)) +
        geom_line(size = 2) +
        scale_color_manual(values = c("#ED8800", "#000000")) +
        labs(title = paste0("Performance comparison of ", model1, " and\n", model2), x = "false positive rate", y = "true positive rate") +
        theme_minimal() +
