...
cm_nb_ut <- data.frame(as.matrix(model_build_cm_nb$table)) %>%
...
cm_lr_ut <- data.frame(as.matrix(model_build_cm_lr$table)) %>%
        mutate(pct = Freq / sum(Freq, na.rm = TRUE),
               outcome = ifelse(Reference == Prediction, "correct", ifelse(Reference == "active" & Prediction == "termed", "false negative", ifelse(Reference == "termed" & Prediction == "active", "false positive", "unknown"))),
               algorithm = "logistic_regression")

cm_rf_ut <- data.frame(as.matrix(model_build_cm_rf$table)) %>%
        mutate(pct = Freq / sum(Freq, na.rm = TRUE),
               outcome = ifelse(Reference == Prediction, "correct", ifelse(Reference == "active" & Prediction == "termed", "false negative", ifelse(Reference == "termed" & Prediction == "active", "false positive", "unknown"))),
               algorithm = "random_forest")

cm_rt_ut <- data.frame(as.matrix(model_build_cm_rt$table)) %>%
        mutate(pct = Freq / sum(Freq, na.rm = TRUE),
               outcome = ifelse(Reference == Prediction, "correct", ifelse(Reference == "active" & Prediction == "termed", "false negative", ifelse(Reference == "termed" & Prediction == "active", "false positive", "unknown"))),
               algorithm = "regression_tree")

confusion_matrix_untrained <- bind_rows(cm_nb_ut, cm_lr_ut, cm_rf_ut, cm_rt_ut)
names(confusion_matrix_untrained) <- c("predicted", "actual", "n_ut", "pct_ut", "outcome", "algorithm")
...
confusion_matrix_untrained_w <- merge(x = cm_lr_ut[ , c(1:5)], y = cm_nb_ut[ , c(1:5)], by = c("actual", "predicted", "outcome"), all.x = TRUE, all.y = TRUE)
confusion_matrix_untrained_w <- merge(x = confusion_matrix_untrained_w, y = cm_rf_ut[ , c(1:5)], by = c("actual", "predicted", "outcome"), all.x = TRUE, all.y = TRUE)
confusion_matrix_untrained_w <- merge(x = confusion_matrix_untrained_w, y = cm_rt_ut[ , c(1:5)], by = c("actual", "predicted", "outcome"), all.x = TRUE, all.y = TRUE)

confusion_matrix_untrained_w <- confusion_matrix_untrained_w %>%
        arrange(outcome, actual, predicted)


names(confusion_matrix_untrained) <- c("predicted", "actual", "n_ut", "pct_ut", "outcome", "algorithm")

confusion_matrix_untrained <- confusion_matrix_untrained %>%
        dplyr::select(actual, predicted, outcome, n_ut, pct_ut, algorithm) %>%
        arrange(algorithm, outcome, actual, predicted)

confusion_matrix_untrained_w %>%
        gt() %>%
...
confusion_matrix_untrained$phase <- "untrained"
...
names(perf_metrics_lr_untrained) <- c("metric", "metric_result_lr", "algorithm", "phase")
names(perf_metrics_nb_untrained) <- c("metric", "metric_result_nb", "algorithm", "phase")
names(perf_metrics_rf_untrained) <- c("metric", "metric_result_rf", "algorithm", "phase")
names(perf_metrics_rt_untrained) <- c("metric", "metric_result_rt", "algorithm", "phase")

perf_metrics_untrained_w_df <- merge(x = perf_metrics_lr_untrained[ , c(1:2)], y = perf_metrics_nb_untrained[ , c(1:2)],
                                     by = "metric", all.x = TRUE, all.y = TRUE)
perf_metrics_untrained_w_df <- merge(x = perf_metrics_untrained_w_df, y = perf_metrics_rf_untrained[ , c(1:2)],
                                     by = "metric", all.x = TRUE, all.y = TRUE)
perf_metrics_untrained_w_df <- merge(x = perf_metrics_untrained_w_df, y = perf_metrics_rt_untrained[ , c(1:2)],
                                     by = "metric", all.x = TRUE, all.y = TRUE)

perf_metrics_untrained_w_df %>%
        gt() %>%
...
# Build the models
model_test_nb_df <- train(Attrition ~ ., data = train_data_df[, -1], method = "nb",
                          trControl = trainControl("cv", number = 10))
model_test_lr_df <- train(Attrition ~ ., data = train_data_df[, -1], method = "glm", family = "binomial",
                          trControl = trainControl("cv", number = 10))
model_test_rf_df <- train(Attrition ~ ., data = train_data_df[, -1], method = "rf",
                          trControl = trainControl("cv", number = 10))
model_test_rt_df <- train(Attrition ~ ., data = train_data_df[, -1], method = "rpart",
                          trControl = trainControl("cv", number = 10))
...
model_trained_cm_nb <- confusionMatrix(predicted_classes_nb, test_data_df$Attrition)
model_trained_cm_lr <- confusionMatrix(predicted_classes_lr, test_data_df$Attrition)
model_trained_cm_rf <- confusionMatrix(predicted_classes_rf, test_data_df$Attrition)
model_trained_cm_rt <- confusionMatrix(predicted_classes_rt, test_data_df$Attrition)

cm_nb_t <- data.frame(as.matrix(model_trained_cm_nb$table)) %>%
        mutate(algorithm = "naive_bayes")

cm_lr_t <- data.frame(as.matrix(model_trained_cm_lr$table)) %>%
        mutate(algorithm = "logistic_regression")

cm_rf_t <- data.frame(as.matrix(model_trained_cm_rf$table)) %>%
        mutate(algorithm = "random_forest")

cm_rt_t <- data.frame(as.matrix(model_trained_cm_rt$table)) %>%
        mutate(algorithm = "regression_tree")

confusion_matrix_trained <- bind_rows(cm_nb_t, cm_lr_t, cm_rf_t, cm_rt_t)
names(confusion_matrix_trained) <- c("predicted", "actual", "n_tr", "algorithm")

confusion_matrix_trained <- confusion_matrix_trained %>%
        group_by(algorithm) %>%
...
confusion_matrix_trained$phase <- "trained"

confusion_matrix_combo <- merge(x = confusion_matrix_untrained[, c(1:6)], y = confusion_matrix_trained[, c(1:6)], by = c("actual", "predicted", "outcome", "algorithm"), all.x = TRUE, all.y = TRUE)
...
confusion_matrix_combo %>%
        gt(group = "algorithm") %>%
        tab_spanner(label = md("*Untrained*"), columns = vars(n_ut, pct_ut)) %>%
        tab_spanner(label = md("*Trained*"), columns = vars(n_tr, pct_tr)) %>%
...
Notice that XXXX has gone to 0 for the naive Bayes model. That is because it predicted that all employees would remain active--as we saw above.
...
metric_labels <- row.names(as.data.frame(c(model_trained_cm_nb$overall, model_trained_cm_nb$byClass)))

perf_metrics_nb_trained <- data.frame(metric = metric_labels,
                                        metric_result = c(model_trained_cm_nb$overall, model_trained_cm_nb$byClass)) %>%
        mutate(algorithm = "naive_bayes",
               phase = "trained")

perf_metrics_lr_trained <- data.frame(metric = metric_labels,
                                        metric_result = c(model_trained_cm_lr$overall, model_trained_cm_lr$byClass)) %>%
        mutate(algorithm = "logistic_regression",
               phase = "trained")

perf_metrics_rf_trained <- data.frame(metric = metric_labels,
                                        metric_result = c(model_trained_cm_rf$overall, model_trained_cm_rf$byClass)) %>%
        mutate(algorithm = "random_forest",
               phase = "trained")

perf_metrics_rt_trained <- data.frame(metric = metric_labels,
                                        metric_result = c(model_trained_cm_rt$overall, model_trained_cm_rt$byClass)) %>%
        mutate(algorithm = "regression_tree",
               phase = "trained")

perf_metrics_trained_df <- bind_rows(perf_metrics_nb_trained, perf_metrics_lr_trained, perf_metrics_rf_trained,
                                       perf_metrics_rt_trained)

names(perf_metrics_lr_trained) <- c("metric", "metric_result_lr_tr", "algorithm", "phase")
names(perf_metrics_nb_trained) <- c("metric", "metric_result_nb_tr", "algorithm", "phase")
names(perf_metrics_rf_trained) <- c("metric", "metric_result_rf_tr", "algorithm", "phase")
names(perf_metrics_rt_trained) <- c("metric", "metric_result_rt_tr", "algorithm", "phase")

perf_metrics_trained_w_df <- merge(x = perf_metrics_lr_trained[ , c(1:2)], y = perf_metrics_nb_trained[ , c(1:2)],
                                     by = "metric", all.x = TRUE, all.y = TRUE)
perf_metrics_trained_w_df <- merge(x = perf_metrics_trained_w_df, y = perf_metrics_rf_trained[ , c(1:2)],
                                     by = "metric", all.x = TRUE, all.y = TRUE)
perf_metrics_trained_w_df <- merge(x = perf_metrics_trained_w_df, y = perf_metrics_rt_trained[ , c(1:2)],
                                     by = "metric", all.x = TRUE, all.y = TRUE)

perf_metrics_df <- merge(x = perf_metrics_untrained_w_df, y = perf_metrics_trained_w_df, by = "metric", all.x = TRUE, all.y = TRUE) %>%
...
perf_metrics_df %>%
        ...
        cols_label(metric_result_lr = "untrained", metric_result_lr_tr = "trained",
                   metric_result_nb = "untrained", metric_result_nb_tr = "trained",
                   metric_result_rf = "untrained", metric_result_rf_tr = "trained",
                   metric_result_rt = "untrained", metric_result_rt_tr = "trained") %>%
...
Now let's take a look at the goodness-of-fit measures for all importance threshold cutoff points. The goodness-of-fit measures we will analyze are:  1) likelihood ratio test; 2) pseudo R-squared; and 3) the Hosmer-Lemeshow Test.
